  ...
  return (
    <>
      ...
      <Section ground="ink">
        ...
        <ul className="mt-12 grid gap-px bg-bone/15 sm:grid-cols-2 lg:grid-cols-3">
          {pillars.map((p, i) => (
            <li key={p} className="flex items-center gap-4 bg-counsel-ink p-6">
              <span className="font-mono text-sm text-aurum">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="text-[1.05rem] text-bone/90">{p}</span>
            </li>
          ))}
        </ul>
      </Section>
      ...
      <Section ground="bone">
        <div className="border border-counsel-ink/15 bg-bone/50 p-8 sm:p-12">
          <div className="flex items-center gap-4">
            <Mark size={32} className="text-aurum" />
          ...
          <p className="mt-4 max-w-2xl text-[1.05rem] leading-relaxed text-counsel-ink/80">
